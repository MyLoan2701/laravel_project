    ...
    <div class="filter-module-container">
        <div id="filter_module">
            <div class="filter-list">
                ...
                <div>
                    <button class="filter-wrapper btn-filter type-sort"><!--<i class="fa-solid fa-mobile"></i>--> <span>Loại {{$nameP->name_brand}}</span> <i style="margin-left: 6px;" class="fa-solid fa-caret-down"></i></button>
                    <div class="list-filter-child" data-index="1">
                        <ul>
                            @foreach($type as $b)
                            @if($b->parent_brand == $nameP->id_brand)
                            <li>
                                <button class="btn-filter-item type-sort-item" data-id-brand="{{$b->id_brand}}" data-href="{{$b->slug_brand}}" onclick="urlSort('type', this.getAttribute('data-href'))">{{$b->name_brand}}</button>
                            </li>
                            @endif
                            @endforeach
                        </ul>
                        <div class="filter-close"><button class="btn-filter-close btn-danger" data-index="1" style="width: 100%;">Đóng</button></div>
                    </div>
                </div>
            </div>
        </div>
...
<div class="category-list">
    ...
    <a href="{{URL::to('/category/'.$lc->slug_category)}}" class="link-category">
        <img src="{{URL::to('public/upload/category/'.$lc->img_category)}}" alt="{{$lc->img_category}}" class="img-category">
    </a>
    @endforeach
</div>
<div class="features_items"><!--features_items-->
    <h2 class="title text-center">Sản phẩm loại {{$nameP->name_brand}}</h2>
